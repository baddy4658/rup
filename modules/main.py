...
# Constants
OWNER_IDS = [7310262552]  # Replace with the actual owner user IDs

# Global variables
log_channel_id = -1002266296818
authorized_users = []
ALLOWED_CHANNEL_IDS = []
my_name = "⏤͟͞𝘽𝘼𝘿𝘿𝙔❤️"
overlay = None 
...
#Cookies files information by Baddy
...
#===================== Cookies update  handler ===============================

@bot.on_message(filters.command("cookies") & filters.create(owner_filter))
async def request_cookies(client, message):
    if message.from_user.id == OWNER_IDS: 
        ...
        new_message = await bot.listen(message.chat.id, timeout=30)
...
async def start(client: Client, msg: Message):
    ...
    if msg.from_user.id in authorized_users:
        ...
        await start_message.edit_text(
            ...
            "Want to get started? 𝗣𝗥𝗘𝗦𝗦 /id\n\n💬 Contact @BADDY_15 to Get The Subscription 🎫 and unlock the full potential of your new bot! 🔓"
        )
...
@bot.on_message(filters.command(["baddy"]))
...
#===============================================================
    if raw_text5 == "df":
        ...
        raw_text2 = "480"
        res = "854x720"
        CR = '<a href="https://t.me/BADDY_15">⏤͟͞𝘽𝘼𝘿𝘿𝙔 ❤️</a>'
        raw_text4 = "df"
        thumb = "no"       
    ...
    for i in range(count - 1, end_count):
        ...
        try:
            ...
            if '/master.mpd' in url and not '/drm/' in url:
                id = url.split("/")[-2]
                url = f'https://pw-url-api-v1mf.onrender.com/process?v=https://sec1.pw.live/{id}/master.mpd&quality={raw_text2}'
                print(url)   
...
#===============================================================
            if raw_text4 == "YES":
                ...
                cc = f'**——— ✦ {str(count).zfill(3)}. ✦ ———**\n\n**🎞️ Title** : **{name1}** - ({res}) {my_name}.mkv\n\n**📚 Course** : `{b_name}`\n\n**🌟 Extracted By : {CR}**'
                cc1 = f'**——— ✦ {str(count).zfill(3)}. ✦ ———**\n\n**🎞️ Title** : **{name1}** - {my_name}.pdf\n\n**📚 Course** : `{b_name}`\n\n**🌟 Extracted By : {CR}**'                             
...
4. **/baddy** - 💡 Type this before sending your **📃.txt** file.
...
Feel free to contact @BADDYxBOT for further assistance or subscription details.
...
12. **/baddy** - 💡 Type this before sending your **📃.txt** file.
...
BUTTONS = InlineKeyboardMarkup([[InlineKeyboardButton(text="Send Here", url=f"https://t.me/BADDYxBOT")]])
